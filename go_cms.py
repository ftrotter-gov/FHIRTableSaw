#!/usr/bin/env python3
"""Runner for CMS FHIR data.

This script loads the CMS configuration and runs the main go.py pipeline.
...
def main(argv: list[str] | None = None) -> int:
    """Main entry point for CMS processing."""
    # Load environment variables
    load_dotenv(override=True)
    
    # Get CMS configuration
    try:
        data_dir, schema = get_data_source_config(source="cms")
    except ValueError as e:
        print(f"ERROR: {e}", file=sys.stderr)
        print("\nPlease ensure CMS_FHIR_DIR and CMS_FHIR_SCHEMA are set in your .env file.", file=sys.stderr)
        return 1
    ...
    print("=" * 80)
    print("CMS FHIR Data Processing")
    print("=" * 80)
