#!/usr/bin/env python3
"""Runner for Palantir FHIR data.

This script loads the Palantir configuration and runs the main go.py pipeline.
...
def main(argv: list[str] | None = None) -> int:
    """Main entry point for Palantir processing."""
    # Load environment variables
    load_dotenv(override=True)
    
    # Get Palantir configuration
    try:
        data_dir, schema = get_data_source_config(source="palantir")
    except ValueError as e:
        print(f"ERROR: {e}", file=sys.stderr)
        print("\nPlease ensure PALANTIR_FHIR_DIR and PALANTIR_FHIR_SCHEMA are set in your .env file.", file=sys.stderr)
        return 1
    ...
    print("=" * 80)
    print("Palantir FHIR Data Processing")
    print("=" * 80)
