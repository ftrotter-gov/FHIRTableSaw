#!/usr/bin/env python3
"""
SAFE FHIR NDJSON to Neo4j import/update script using MERGE (idempotent).

This script uses MERGE for safe, idempotent imports that can update existing data.
It's slower than import_ndjson_fast.py but safe to run multiple times.

Use this script when:
- Updating existing data
- You're not sure if data already exists
- Safety is more important than speed

For initial loads into an empty database, use import_ndjson_fast.py instead (10-100x faster).

Usage:
    python import_or_update_ndjson.py /path/to/ndjson/directory [options]

Example:
    python import_or_update_ndjson.py /data/fhir --batch-size 5000
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Import/update all FHIR resources from a directory
  python import_or_update_ndjson.py /data/fhir
  
  # Use custom batch size
  python import_or_update_ndjson.py /data/fhir --batch-size 500
  
  # Tag this import run (useful for tracking data sources)
  python import_or_update_ndjson.py /data/fhir --import-tag "update_2026"
  
  # Limit records for testing (import only first 10 of each resource type)
  python import_or_update_ndjson.py /data/fhir --limit 10
