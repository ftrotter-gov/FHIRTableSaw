#!/usr/bin/env python3
"""
Script to load Endpoint NDJSON data using fhir4ds.

This demonstrates the SQL on FHIR approach using ViewDefinitions.

Usage:
    python scripts/load_endpoint_ndjson.py <path_to_ndjson_file>
...
from fhir_tablesaw_3tier.fhir4ds_integration import process_endpoint_ndjson
...
def main() -> None:
    ...
    if len(sys.argv) < 2:
        ...
        print()
        print("Example:")
        print(f"  python {sys.argv[0]} /Volumes/eBolt/palantir/ndjson/initial/Endpoint.5.ndjson")
        sys.exit(1)

    ndjson_path = sys.argv[1]

    print(f"Loading Endpoint data from: {ndjson_path}")
    print()

    try:
        result = process_endpoint_ndjson(
        ...
        print(f"Matching Endpoints: {result.get('matching_resources', 'N/A')}")
