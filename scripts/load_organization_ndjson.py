#!/usr/bin/env python3
"""
Script to load Organization NDJSON data using fhir4ds.

This demonstrates the SQL on FHIR approach using ViewDefinitions.
Filters for clinical organizations (type='prov').

Usage:
    python scripts/load_organization_ndjson.py <path_to_ndjson_file>
...
from fhir_tablesaw_3tier.fhir4ds_integration import process_organization_ndjson
...
def main() -> None:
    ...
    if len(sys.argv) < 2:
        ...
        print("Example:")
        print(
            f"  python {sys.argv[0]} /Volumes/eBolt/palantir/ndjson/initial/Organization.5.ndjson"
        )
        sys.exit(1)

    ndjson_path = sys.argv[1]

    print(f"Loading Organization data from: {ndjson_path}")
    print("NOTE: Filtering for clinical organizations (type='prov')")
    print()

    try:
        result = process_organization_ndjson(
        ...
        print(f"Matching Organizations: {result.get('matching_resources', 'N/A')}")
