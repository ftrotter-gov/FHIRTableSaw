#!/usr/bin/env python3
"""
Script to load PractitionerRole NDJSON data using fhir4ds.

This demonstrates the SQL on FHIR approach using ViewDefinitions.

Usage:
    python scripts/load_practitioner_role_ndjson.py <path_to_ndjson_file>
...
from fhir_tablesaw_3tier.fhir4ds_integration import process_practitioner_role_ndjson
...
def main() -> None:
    ...
    if len(sys.argv) < 2:
        ...
        print("Example:")
        print(
            f"  python {sys.argv[0]} /Volumes/eBolt/palantir/ndjson/initial/PractitionerRole.5.ndjson"
        )
        sys.exit(1)

    ndjson_path = sys.argv[1]

    print(f"Loading PractitionerRole data from: {ndjson_path}")
    print()

    try:
        result = process_practitioner_role_ndjson(
        ...
        print(f"Matching PractitionerRoles: {result.get('matching_resources', 'N/A')}")
